...
表示の自動生成は、設定済み特殊性能の実処理への接続を全件保証するものではありません。2026-09-16の確認で、以下は設定と表示のみで、実処理への接続が見つかっていません。別の変更で修正・検証します。

- スピードトレイの `firstTurnSpdBonus` / `afterTurnSpdPenalty`。
- おまじないステッキの `healMagicRate`。現状の回復量計算は `healRate` のみ参照。
- 癒しのエプロンの `statusHealChance`。
